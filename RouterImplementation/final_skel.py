...
class final_topo(Topo):
  def build(self):
    ...
    srvr = self.addHost('srvr',mac='00:00:00:00:00:04',ip='10.0.4.104/24', defaultRoute="srvr-eth0")
    evil = self.addHost('evil',mac='00:00:00:00:00:05',ip='128.114.50.10/24', defaultRoute="evil-eth0")
    ...
    self.addLink(s5,srvr, port1=8, port2=0)

    self.addLink(s4,evil, port1=9, port2=0)

    self.addLink(s4,s1, port1=5, port2=9)
    self.addLink(s4,s2, port1=4, port2=9)
    self.addLink(s4,s3, port1=3, port2=9)
    self.addLink(s4,s5, port1=1, port2=9)
def configure():
  topo = final_topo()
